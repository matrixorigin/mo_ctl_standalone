...
{
    option="precheck"
    echo "Usage         : $0 ${option} # ${USAGE_PRECHECK}"
    echo -n "   Check list : "
...
{
    option="deploy"
    echo "Usage         : $0 ${option} [mo_version] [force] # ${USAGE_DEPLOY}"
    ...
    echo "  e.g.        : $0 ${option}             # default, same as $0 ${option} ${MO_DEFAULT_VERSION}"
    echo "              : $0 ${option} main        # deploy development latest version"
    echo "              : $0 ${option} d29764a     # deploy development version d29764a"
    echo "              : $0 ${option} 0.8.0       # deploy stable verson 0.8.0"
    echo "              : $0 ${option} force       # delete all under MO_PATH and deploy verson ${MO_DEFAULT_VERSION}"
    echo "              : $0 ${option} 0.8.0 force # delete all under MO_PATH and deploy stable verson 0.8.0 from beginning"
}
...
{
    option="status"
    echo "Usage         : $0 ${option} # ${USAGE_STATUS}"
}
...
{
    option="start"
    echo "Usage         : $0 ${option} # ${USAGE_START}"
}

function help_stop()
{
    option="stop"
    echo "Usage         : $0 ${option} [force] # ${USAGE_STOP}"
    echo " [force]      : optional, if specified, will try to kill mo-services with -9 option, so be very carefully"
    echo "  e.g.        : $0 ${option}         # default, stop all mo-service processes found on this machine"
    echo "              : $0 ${option} force   # stop all mo-services with kill -9 command"
}
...
{
    option="restart"
    echo "Usage         : $0 ${option} # ${USAGE_RESTART}"
}

function help_connect()
{
    option="connect"
    echo "Usage         : $0 ${option} # ${USAGE_CONNECT}"
}

function help_get_cid()
{
    option="get_cid"
    echo "Usage         : $0 ${option} # ${USAGE_GET_CID}"
}

function help_path()
{
    option="path"
    echo "Usage         : $0 ${option} # ${USAGE_PATH}"
}
...
{
    option="pprof"
    echo "Usage         : $0 ${option} [item] [duration] # ${USAGE_PPROF}"
    ...
    echo "  e.g.        : $0 ${option}"
    echo "              : $0 ${option} profile    # collect duration will use conf value PPROF_PROFILE_DURATION from conf file or 30 if it's not set"
    echo "              : $0 ${option} profile 30"
    echo "              : $0 ${option} heap"
}
...
{
    option="setconf"
    echo "Usage         : $0 ${option} [conf_list] # ${USAGE_SET_CONF}"
    echo " [conf_list]  : configuration list in key=value format, seperated by comma"
    echo "  e.g.        : $0 ${option} MO_PATH=/data/mo/20230629/matrixone,MO_PW=M@trix0riginR0cks,MO_PORT=6101"
    echo "              : $0 ${option} MO_PATH=/data/mo/20230629/matrixone"
}
...
{
    option="ddl_convert"
    echo "Usage           : $0 ${option} [options] [src_file] [tgt_file] # ${USAGE_DDL_CONVERT}"
    ...
    echo "  e.g.          : $0 ${option} mysql_to_mo /tmp/mysql.sql /tmp/mo.sql"
}

function help_1()
{
    echo "Usage             : $0 [option_1] [option_2]"
    echo ""
    ...
    echo "  e.g.            : $0 status"
    echo ""
    echo "[option_2]        : Use \" $0 [option_1] help \" to get more info"
    echo "  e.g.            : $0 deploy help "
}
